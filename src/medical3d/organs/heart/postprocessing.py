"""Heart postprocessing: keep the single largest component, fill holes, close.

The level set produces one contiguous blob by construction (it grows from a
single seed), so postprocessing here is purely cleanup — small stray voxels
from thresholding the level set at 0 near-boundary, and closing to smooth
the surface before meshing.
"""
