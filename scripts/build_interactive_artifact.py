...
<script>
  ...
  function computeGradientField() {
    ...
    const strideX = Math.max(1, Math.round(nx / 26));
    const strideY = Math.max(1, Math.round(ny / 26));
    const strideZ = Math.max(1, Math.round(nz / 26));
    const stepMm = Math.max(sx, sy, sz) * 0.9;
    const STEPS = 4; // per direction from the seed, so up to 8 short segments form one flowing streamline
